...
int32_t NoiseReduce_Init(NoiseReduce* handle, uint16_t sample_rate, uint16_t fftlen, uint8_t bPostFilt)
{
	...
	handle->Ypw = (float*)malloc(half_fftlen*sizeof(float));
	handle->Npw = (float*)malloc(half_fftlen*sizeof(float));
	handle->spp = (float*)malloc(half_fftlen*sizeof(float));

	uiv_fill_f32(0.f, handle->Ypw, half_fftlen);
	uiv_fill_f32(0.f, handle->Npw, half_fftlen);
	uiv_fill_f32(0.5f, handle->spp, half_fftlen);

	//handle->speech_bin = (uint32_t*)malloc(hMemMgr, half_fftlen);
	//handle->noise_bin = (uint32_t*)malloc(hMemMgr, half_fftlen);

#ifdef _NS_ENABLE
	if (handle->bPostFilt)
	{
		handle->last_Ypw = (float*)malloc(half_fftlen*sizeof(float));
		handle->last_Npw = (float*)malloc(half_fftlen*sizeof(float));
		handle->post_snr = (float*)malloc(half_fftlen*sizeof(float));
		uiv_fill_f32(0.f, handle->last_Ypw, half_fftlen);
		uiv_fill_f32(0.f, handle->last_Npw, half_fftlen);
		uiv_fill_f32(0.f, handle->post_snr, half_fftlen);
	}
#endif
...
int32_t NoiseReduce_EstimateNoise(NoiseReduce* handle, float* ref_power, uint32_t frame_cnt, uint8_t cep_vad)
{
	uint16_t idx;
	...
	if (handle->bPostFilt)
		uiv_copy_f32(handle->Npw, handle->last_Npw, half_fftlen);
...
int32_t NoiseReduce_WienerFilter(NoiseReduce* handle, float* input, float* output)
{
	...
	for (idx = 0; idx < half_fftlen; ++idx)
	{
		...
		handle->last_Ypw[idx] = (output[2 * idx] * output[2 * idx] + output[2 * idx + 1] * output[2 * idx + 1]);
	}
